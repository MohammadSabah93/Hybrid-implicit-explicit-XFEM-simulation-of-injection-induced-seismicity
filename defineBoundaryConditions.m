function boundaryConditions = defineBoundaryConditions(R,E)
    ...
    rollerXNodes = [boundaryConditions.leftEdgeNodes];
    rollerXDOFs = 2*rollerXNodes - 1; % fix only u_x
    rollerXDisp = zeros(length(rollerXDOFs),1);

    % RollerYNodes 
    rollerYNodes = [boundaryConditions.bottomEdgeNodes];
   ...
   % Traction boundaries
    ...
    tractionConfig.rightEdge=[-40e+6,0];
    tractionConfig.topEdge=[0,-20e+6];
...
% Flux boundaries
    ...
    boundaryConditions.concentratedSources = [5403,4e-4];
    % Displacement DOFs
