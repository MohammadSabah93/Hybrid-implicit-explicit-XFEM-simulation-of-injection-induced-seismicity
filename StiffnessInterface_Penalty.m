function globalK = StiffnessInterface_Penalty(params, connectivity, XYZ, NODES, CRACK, n, t, Sn, duf, dt, stickflag, elementType, PHI, omega, domain_length, nex)
...
if stickflag(counter,1)==0
   D = Kn * (n*n') + Ks * (eye(2)-n*n');
else
  
   D = Kn * (n*n') + t * (Ks * (t - (Ks*t)/(Ks - duf(counter,1) * Sn(counter,1) + damp/dt)))';
    ...
    localK = localK + 4 * gw(gpIndex) * Nstd' * D * Nstd * det(jacobian);
    
     end
    globalK(local,local) = globalK(local,local) + localK; 
    counter = counter +1;
end
end
end
